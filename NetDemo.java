...
protected void commandControl(KeyEvent ke){
	...
	if(c=='\n'){
	 ...
	 else if(commandEnter.startsWith("find")||commandEnter.startsWith("find"))
	 {
		...
		ta.append(commandEnter +"\n");
		String token = tokenizer.nextToken();
		String ta_FileExist="" , ta_FileDoesNotExist="";
		...
		while(tokenizer.hasMoreTokens()){
			...
			try{
				...
				if(findFile==true)
				{
					ta_FileExist=ta_FileExist+token+"\n";
				}
				else {
		...
		ta.append("The files(s) exist(s):\n");
		ta.append(ta_FileExist);
	 ...
	 //////////////////DIRECTORY////////////
		 ...
		 try{
			 ...
			 for(j=0;j<fileNames.length;j++){
				 ...
				 ta.append(fileNames[j]+","+"("+fileSize+"byte)\n");
			 }
	 ...
	 else if(commandEnter.startsWith("copy")||commandEnter.startsWith("copy"))
	 {
		 ...
		 try{
			 ...
			 ta.append("can't execute copy command\n");
		 }
	...
	else if(commandEnter.startsWith("edit")||commandEnter.startsWith("edit"))
	{
		...
		//ta.setText();
		editOn=true;
		myFrame.setTitle("Net demo Console - Edit Mode(Type 'exit' to quit)-"+currentFile);
		ta.requestFocus();
	...
	////////////////HELP///////////////
		...
		ta.append("date   |Shows Date     |run calc notepad word prog \n");
		ta.append("cls   |Clear Screen    |cls\n");
		ta.append("cd     |Change directory    |cd program\n");
		ta.append("copy    |Change directory    |copy read.txt newreadme.txt\n");
		ta.append("dir    |show directory    |dir\n");
		ta.append("memcheck   |Checks Memory   |memcheck \n");
		ta.append("prompt   |changes prompt sign  |prompt>>>\n");
	    ta.append("urlinfo  |Gives urlinfo    |urlinfo http://www.yahoo.com\n");
	    ta.append("sort   |Sort given data   |sort bad perfect great\n");
		ta.append("edit   |goes edit mode   |edit file name(optional)\n");
		ta.append("find   |Finds file   |find prog.dbm hello.txt\n");
		ta.append("kill   |Delete file   |kill file1.txt file2.txt \n");
		ta.append("save  |save file in edit mode  |save file1 file2 file 3\n");
		ta.append("local  |shows country time & Date |local \n");
		ta.append("nv   |Normal vedio   |nv\n");
		ta.append("bv   |Black Vedio   |bv\n");
		ta.append("gv    |Green Vedio   |gv\n");
		ta.append("ov    |Orange  Vedio   |ov\n");
		ta.append("yv    |Yellow Vedio   |yv\n");
		ta.append("ren     |Rename file     |ren oldName.txt\n");
		ta.append("md    |Create new Directory    |md new Directory\n");
		ta.append("show    |Show file content   |show readme.txt\n");
		ta.append("osinfo  |OS Information    |osinfo\n");
		ta.append("exit   |Exit from Edit Mode   |exit\n");
		ta.append("help   |Show help information   |help\n");
		ta.append("?   |Show help information  |?\n");
